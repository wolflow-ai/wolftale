...
Conflict classification (no API call — structural only):
  - similarity >= DUPLICATE_THRESHOLD (0.85): duplicate
  - similarity >= CONFLICT_THRESHOLD (0.70) + same domain: potential contradiction, flag
  - similarity >= CONFLICT_THRESHOLD (0.70) + different domain: update, flag lightly
...
DUPLICATE_THRESHOLD = 0.85   # Cosine similarity — skip write, return deduplicated
CONFLICT_THRESHOLD = 0.70    # Cosine similarity — write but flag
...
def _classify_conflict(
    ...
    Rules:
      ...
      similarity >= CONFLICT_THRESHOLD + same domain  → contradiction
      similarity >= CONFLICT_THRESHOLD + diff domain  → update
    """
